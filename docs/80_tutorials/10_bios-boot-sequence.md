...
To build the `eosio.boot` located in the `eos` repository, create a dedicated directory for `eos`, clone the `eos` repository and build the `eosio.boot` system smart contract sources.
