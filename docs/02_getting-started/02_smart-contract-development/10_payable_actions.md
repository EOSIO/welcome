## Goal
This tutorial will illustrate how to write a smart contract that has payable actions. Payable actions are actions that require you to transfer some tokens to actions prior to using other functionality of the smart contract. This tutorial will also go through the concept of an `asset` type.
...
[[eosio::on_notify("eosio.token::transfer")]]
```
The `on_notify` attribute is one of the EOSIO.CDT [attributes](https://eosio.github.io/eosio.cdt/1.6.0/guides/generator-attributes.html) that annotates a smart contract action.
...
```shell
cleos create account eosio hodl DEVELOPMENT_KEY
...
```
Use the `eosio.token` deployed in the previous section to issue some SYS tokens to user `han`:

```shell
cleos push action eosio.token issue '[ "han", "100.0000 SYS", "Slecht geld verdrijft goed" ]' -p eosio@active
```
