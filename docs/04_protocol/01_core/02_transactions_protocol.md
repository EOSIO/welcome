...
A packed transaction is simply a signed transaction with an optional data compression field. This is intended to minimize transaction footprint and, hence, minimize block size in the long run. A packed transaction forms the most inclusive type of transaction in an EOSIO blockchain. Consequently, when transactions are pushed to a block, they are actually packed transactions.
