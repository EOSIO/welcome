...
```
The first new concept in the code above is that we are explicitly restricting calls to the one action to a **specific account** in this contract using [require_auth](https://developers.eos.io/manuals/eosio.cdt/v1.7/group__action/#function-require_auth) to the `addressbook` contract, as seen below.
```cpp
...
Another new concept in the code above, is [action wrapper](https://developers.eos.io/manuals/eosio.cdt/v1.7/structeosio_1_1action__wrapper). As shown below the first template parameter is the 'action' we are going to call and the second one should point to the action function
```text
using count_action = action_wrapper<"count"_n, &abcounter::count>;
```
...
This time we use the [action wrapper](https://developers.eos.io/manuals/eosio.cdt/v1.7/structeosio_1_1action__wrapper) instead of calling a function. To do that, we firstly initialised the count_action object defined earlier. The first parameter we pass is the callee contract name, in this case `abcounter`. The second parameter is the permission struct.

- For the permission, [get_self()](https://developers.eos.io/manuals/eosio.cdt/v1.7/classeosio_1_1contract/#function-get_self)  returns the current `addressbook` contract. The `active` permission of `addressbook` is used.
