...
- A [permission_level](https://developers.eos.io/manuals/eosio.cdt/v1.7/structeosio_1_1permission__level) struct
...
Since the action called is in this contract, use  [get_self](https://developers.eos.io/manuals/eosio.cdt/v1.7/classeosio_1_1contract/#function-get_self). `"addressbook"_n` would also work here, but if this contract were deployed under a different account name, it wouldn't work. Because of this, `get_self()` is the superior option.
```cpp
...
  private:
    void send_summary(name user, std::string message){
