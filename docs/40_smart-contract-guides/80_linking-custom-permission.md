...
EOSIO permissions control what an account is authorized to execute on an EOSIO blockchain. EOSIO permissions are powerful, flexible and customizable. This tutorial explains custom permissions and shows you how to use them. In the tutorial you will create a custom permission; then link the permission to an action.
## Before You Begin
...
* Some knowledge of smart contracts, we recommend following the [hello world tutorial](../30_getting-started-guide/25_hello-world.md) tutorial
...
In this example you will create two custom permissions on the _bob_ account:
...
* Link _customp1_ to `what(eosio::name user)`
* Link _customp2_ to `how(eosio::name user)`

You will see that the `active` permission can call all the actions.
You will see that the _customp1_  permission can call the `why` and `how` actions.
You will see that the _customp2_  permission can only call the `how` action.
...
Create custom permission _customp1_, with the parent `active`, on the bob account using the command [cleos set account permission](https://developers.eos.io/manuals/eos/latest/cleos/command-reference/set/set-account-permission.md):
...
Now create custom permission _customp2_, with the parent _customp1_, on the bob account:
...
Once you have a custom permission you can link this custom permsssion to a smart contract action, requiring that permission level authorization, or higher, to execute the action.
...
You can unlink permsssions to a smart contract action.
