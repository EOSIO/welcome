...
[/block]
As mentioned earlier, **a table's struct cannot be modified when it has data in it.** The first step is to remove the data we've already added.
...
[/block]
First add a new member variable and its getter to the `addressbook.cpp` contract.

As the secondary index needs to be numeric field, so add an `uint64_t` age variable
[block:code]
{
  "codes": [
    {
...
{
  "type": "info",
  "body": "There are few other allowed types for a secondary index that will be covered in a future tutorial."
}
[/block]
...
[/block]
Now that there is a field that be used as the secondary index, reconfigure the address_index table. 
[block:code]
{
  "codes": [
    {
...
In that `index_by` struct, we specify the name of index as `"byage"` and the second type parameter as a function call operator should extract a const value as an index key. Reference the getter created earlier so this multiple index table will index records by `age` variable.
[block:code]
{
  "codes": [
    {
...
[block:api-header]
{
  "title": "Step 4: Update action's code"
}
[/block]
Now update action code so it can insert new data. 
...
[/block]
Add code that updates or emplace age:
[block:code]
{
  "codes": [
    {
      "code": "if( iterator == addresses.end() ) {\n  addresses.emplace(user, [&]( auto& row ) {\n    ...\n    row.age = age;\n    ...\n  });\n}\nelse {\n  std::string changes;\n  addresses.modify(iterator, user, [&]( auto& row ) {\n    ...\n    row.age = age;\n    ...\n",
      "language": "cplusplus"
    }
  ]
}
...
[/block]
Insert some records to test the changes to the contract
[block:code]
{
  "codes": [
    {
...
[/block]
The complete `addressbook` contract including the new *secondary index* up until this point:
[block:code]
{
  "codes": [
    {
