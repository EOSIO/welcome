---
title: "4.2 Deploying System Contracts"
excerpt: ""
---
...
[block:api-header]
{
  "title": "Step 1: Configure the initial set of `nodeos` nodes"
}
[/block]
# 
...
Resigning involves setting the keys of the `eosio.*` accounts to null, so that this account can no longer be authorized by an individual. 

Clear the keys for both permissions of `eosio` account by using `cleos push action` to `updateauth`
[block:code]
{
  "codes": [
    {
...
[block:code]
{
  "codes": [
    {
      "code": "$ cleos push action eosio updateauth '{\"account\": \"eosio\", \"permission\": \"active\", \"parent\": \"owner\", \"auth\": {\"threshold\": 1, \"keys\": [], \"waits\": [], \"accounts\": [{\"weight\": 1, \"permission\": {\"actor\": \"eosio.prods\", \"permission\": \"active\"}}]}}' -p eosio@active",
      "language": "shell",
      "name": "Clear Active Key"
    }
  ]
}
[/block]