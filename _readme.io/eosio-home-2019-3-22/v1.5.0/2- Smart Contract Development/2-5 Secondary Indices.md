...
[/block]
As mentioned earlier, **a table's struct cannot be modified when it has data in it.** Remove all the data from the `addressbook` contract. Only records for `alice` and `bob` were previously added. 
[block:code]
{
  "codes": [
    {
...
[/block]
Add a new member variable and its getter to the `addressbook.cpp` contract. The secondary index needs to be numeric field, so a `uint64_t` was chosen for the age variable
[block:code]
{
  "codes": [
    {
...
[/block]
Now that a secondary index field has been declared as the secondary index, reconfigure the address_index table. 
[block:code]
{
  "codes": [
    {
...
[/block]
In the third parameter, pass a `index_by` struct which is used to instantiate a index. 

In that `index_by` struct, specify the name of index as `"byage"` and the second type parameter as a function call operator should extract a const value as an index key. In this case, point it to the getter created earlier so this multiple index table will index records by `age` variable.
[block:code]
{
  "codes": [
    {
