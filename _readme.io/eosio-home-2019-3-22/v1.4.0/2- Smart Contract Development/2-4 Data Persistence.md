...
In eosio, a chain has unique accounts, so `name` is an ideal candidate as a **primary_key** in this specific use case. The [name](https://developers.eos.io/eosio-cpp/reference#name) type is a `uint64_t`. 
...
[/block]
Earlier, it was mentioned that only the user has control over their record, as this contract is opt-in. To do this, utilize the [require_auth]() method provided by the `eosio.cdt`. This method accepts one argument, a `name` type, and asserts that the account executing the transaction equals the provided value. 
[block:code]
{
  "codes": [
    {
...
[/block]
Create a record in the table using the multi_index method [emplace](https://developers.eos.io/eosio-cpp/reference#emplace). This method accepts two arguments, the "payer" for the RAM and a callback function. 
...
- The account responsible for paying the ram, in this case, `user`
...
[/block]
A contract *cannot* erase a record that doesn't exist, so assert that it indeed exists before proceeding.
[block:code]
{
  "codes": [
    {
...
## 8.1 EOSIO_DISPATCH
At the bottom of the file, utilize the [EOSIO_DISPATCH]() macro, passing the name of the contract, and the actions. 
