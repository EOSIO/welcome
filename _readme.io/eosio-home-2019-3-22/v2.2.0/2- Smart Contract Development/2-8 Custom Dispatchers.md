...
Below, the `EOSIO_DISPATCH` macro has been expanded to uncover underlying logic. The following dispatcher 
uses `eosio::execute_action`, which allows for flexibility but is still compatible with the dispatcher. As seen below the `EOSIO_DISPATCH` macro is suitable for contracts that handle local actions, but provide no way to handle actions from an external contract. 
[block:code]
{
  "codes": [
    {
...
The expanded dispatcher pattern above is really only suitable when handling internal actions. If your contract needs to handle actions sent by another contract in a specific way, using a `switch()` introduces logical ambiguity which may introduce vulnerabilities. If you like the tradeoffs of this convention, but wish to handle income actions from another contract, see the next example.
