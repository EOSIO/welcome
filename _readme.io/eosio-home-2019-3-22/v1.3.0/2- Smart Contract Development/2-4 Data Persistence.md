...
For this contract, use a field called "key" with type `account_name`. This contract will have one unique entry per user, so this key will be a consistent and guaranteed unique value based on the user's `account_name` 
[block:code]
{
  "codes": [
    {
...
[block:code]
{
  "codes": [
    {
      "code": "typedef eosio::multi_index<N(people), person> address_index;",
      "language": "cplusplus"
    }
  ]
}
[/block]
1. Us the [N](https://developers.eos.io/eosio-cpp/reference#n) macro name the table. This table contains a number of different singular "persons", so name the table "people". 
...
[block:code]
{
  "codes": [
    {
      "code": "addressbook(account_name self): contract(self){}",
      "language": "cplusplus"
    }
  ]
}
...
[/block]
Earlier, it was mentioned that only the user has control over their own record, as this contract is opt-in. To do this, utilize the [require_auth]() method provided by the `eosio.cdt`. This method accepts one argument, an `account_name` type, and asserts that the account executing the transaction equals the provided value. 
[block:code]
{
  "codes": [
    {
      "code": "void upsert(account_name user, std::string first_name, std::string last_name, std::string street, std::string city, std::string state) {\n  require_auth( user );\n}",
      "language": "cplusplus"
    }
  ]
}
...
[block:code]
{
  "codes": [
    {
      "code": "    void erase(account_name user){\n      require_auth(user);\n    }\n",
      "language": "cplusplus"
    }
  ]
}
...
## 8.1 EOSIO_ABI
At the bottom of the file, utilize the [EOSIO_ABI]() macro, passing the name of the contract, and our lone action "upsert". 
...
[block:code]
{
  "codes": [
    {
      "code": "EOSIO_ABI( addressbook, (upsert) )",
      "language": "cplusplus"
    }
  ]
}
...
[block:code]
{
  "codes": [
    {
      "code": "cleos create account eosio addressbook YOUR_PUBLIC_KEY",
      "language": "shell"
    }
  ]
}
...
[block:code]
{
  "codes": [
    {
      "code": "cleos push action addressbook upsert '[\"bob\", \"bob\", \"is a loser\", \"doesn't exist\", \"somewhere\", \"someplace\"]' -p alice@active",
      "language": "text"
    }
  ]
}
...
[block:code]
{
  "codes": [
    {
      "code": "cleos get table addressbook addressbook people -k alice",
      "language": "shell"
    }
  ]
}
