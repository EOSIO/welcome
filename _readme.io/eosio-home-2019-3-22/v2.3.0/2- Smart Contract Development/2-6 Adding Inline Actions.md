...
[/block]
If not still opened, open the `addressbook.cpp` contract authored in the last tutorial. Write an action that will behave as a "transaction receipt" notifier. To do this, create a helper function in the `addressbook` class.
[block:code]
{
  "codes": [
    {
...
[/block]
Now, if anyone other than the contract itself calls this function, an exception will be thrown.
[block:api-header]
{
...
To begin, we'll address the "create record" case. This is the case that fires when a record is not found in the table, i.e. when `iterator == addresses.end()` is `true`.
