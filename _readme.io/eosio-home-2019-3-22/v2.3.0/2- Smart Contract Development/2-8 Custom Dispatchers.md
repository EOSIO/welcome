...
The above pattern is simply an exploded view of the EOSIO_DISPATCH and EOSIO_DISPATCH macros without actually using them, hence the "Exploded EOSIO_DISPATCH" Dispatcher pattern name.

The exploded dispatcher pattern above is really only suitable when handling internal actions. If your contract needs to handle actions sent by another contract in a specific way, using a `switch()` introduces logical ambiguity which may introduce vulnerabilities. If you like the tradeoffs of this pattern, but wish to handle incoming actions from another contract, see the next example.
...
[/block]
The above pattern is the same as the "Exploded EOSIO_DISPATCH" pattern, but utilizes more verbose conditions that are appropriate for handling actions . 
