...
This tutorial is a guide for exchanges that wish to automate deposit and withdrawal of tokens using  standard-conforming EOSIO token contracts. The EOSIO blockchain's native token (eosio.token) conforms to this standard. In this tutorial, the methods for depositing and withdrawing from the exchange account are shown.
...
This tutorial uses the transfer action of the `eosio.token` contract for handling deposits and withdrawals.

For monitoring activity there are two options: 

  * using the history plugin
  * using the state history plugin

## Using the History Plugin

### Ensure `eosio::history_api_plugin` is running

This allows the use of  the `cleos` command line tool to query the log history of accounts. This requires the `eosio::history_api_plugin` to be installed. If your `nodeos` was not started with this plugin, you will need to restart `nodeos` and add `--plugin eosio::history_api_plugin` to the command line.
